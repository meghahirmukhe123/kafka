...
public class userBillControlller {
    ...
    public ResponseEntity<List<userBill>> getAllUserBills()
    {
    	return ResponseEntity.ok().body(service.getAllUserBill());
    }
    
    //delete by email id

	@DeleteMapping("/deleteuserbill/{emailId}")
	public String deleteByEmailId(@PathVariable String emailId) {
		service.deleteBillByEmailID(emailId);
		return "user bill deleted successfully";
	}
